    ...
    <div class="w-full h-[108px] rounded-[6px] bg-[#18162F] flex flex-col p-4 gap-[11px] drop-shadow-md">
        ...
        <div class="flex flex-col">
            {#if $RADIODATA.onRadio}
                ...
                <button class="bg-[#FF453AFF] w-full h-[22px] text-white rounded-[6px] font-bold flex justify-center items-center text-[11px] mt-1" on:click={() => SendNUI('leave')}>{$RADIODATA.locale['ui.disconnect']}</button>
            {:else}
