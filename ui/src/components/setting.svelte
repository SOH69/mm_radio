<script lang='ts'>
    ...
    import { MOVERADIO, RADIODATA } from "@store/stores";
    ...
    function handleSliderRelease() {
        ...
        SendNUI('updateRadioSize', { overlay: overlaySize, radio: radioSize })
    }
...
<div class="w-full h-[254px] drop-shadow-md p-4">
    <div class="w-full h-full bg-[#18162F] rounded-[6px] text-white font-bold overflow-scroll no-scrollbar px-4 py-2">
        <div class="flex flex-col w-full gap-2 border-b-[1px] border-gray-700 pb-4">
            <span class="text-[14px]">Radio Settings</span>
            ...
            <div class="flex gap-2">
                ...
                <span class="text-[10px] flex justify-center items-center">Move Radio</span>
...
<div class="w-full h-[4] px-4 flex gap-2 pb-4">
    <button class="w-full py-3 bg-[#18162F] flex justify-center items-center gap-[11px] text-white text-[11px] rounded-[6px] font-bold drop-shadow-md" on:click={() => saveData()}>{$RADIODATA.locale['ui.save']}</button>
</div>